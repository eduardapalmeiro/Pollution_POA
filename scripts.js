...
function getMap() {
  return window.mapaLeaflet || null;
}

function exibirContador() {
  const el = document.getElementById('ValorContador');
  if (el) el.innerText = vetorDenuncias.length;
}
...
function salvarDenuncia(event) {
  ...
  const map = getMap();
  if (map && typeof renderizarDenunciasNoMapa === 'function') {
    renderizarDenunciasNoMapa(map);
  }
}

function excluirDenuncia(idParaExcluir) {
  vetorDenuncias = vetorDenuncias.filter(d => d.id !== idParaExcluir);
  localStorage.setItem('todasDenuncias', JSON.stringify(vetorDenuncias));
...
function renderizarCards() {
  ...
  vetorDenuncias.forEach(denuncia => {
    ...
    card.style.cssText = ";border:1px solid red;margin:5px 0;padding:8px;border-radius:4px";
    card.innerHTML = `➤ <b>Foco:</b> ${denuncia.endereco} - ${denuncia.bairro}<br>
      <span style="color:#666;font-size:14px">Motivo: ${denuncia.tipo}</span>`;
